...
#---------------------------------------------------------------------------------
#----- RECEPTIONIST SIGN UP AS SYSTEM ADMIN RELATED VIEWS END --------------------
#---------------------------------------------------------------------------------
...
@login_required(login_url='doctorlogin')
@user_passes_test(is_doctor)
def doc_view_patient_view(request):
    patients=models.Patient.objects.all().filter(status=True)
...
def doc_update_patient_view(request,pk):
    ...
    return render(request,'hospital/doc_update_patient.html',context=mydict)
...
def bookappointment_view(request):
    return render(request, 'hospital/Receptionist-book-appointment.html')
